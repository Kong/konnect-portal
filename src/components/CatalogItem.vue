<template>
  <KCard class="show-docs">
    <template #title>
      <p v-if="loading" class="services-card-title">
        <KSkeletonBox width="10" />
      </p>
      <router-link v-else :to="`/spec/${service.id}`">
    ...
    <template #body>
      ...
      <ul class="mt-auto pt1">
        <li class="details-item">
          <span v-if="version" class="my-2 color-text_colors-secondary">
            <template v-if="loading">
        ...
        <li class="docs-links">
          ...
          <div v-if="service.hasDocumentation" class="details-item">
            <template v-if="loading">
              <KSkeletonBox width="50" />
            </template>
            <template v-else>
              <router-link
                :to="{
                  ...
                  params: { service_package: service.id },
                }"
                class="link"
              >
                Documentation
...
.products-content {
  ...
  --KCardBorderRadius: 20px;
  --KCardBorder: 2px solid var(--section_colors-stroke);
