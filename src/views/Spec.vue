<template>
  ...
  >
    ...
    <div v-if="hasServiceError" class="spec-render-error" />

    <div v-else-if="loading" class="spec-loading-container">
      <div>
        <KIcon icon="spinner" size="96" color="var(--steel-300)" />
      </div>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const applicationRegistrationEnabled = computed(() => {
      return (
        currentVersion.value.registration_configs?.length &&
        isAllowedToRegister.value
      );
    });
    ...
    const { sidebarActiveOperation, sidebarOperations } =
      storeToRefs(productStore);

    // converting Operation to OperationListItem
    const convertOperationToListItem: ComputedGetter<
    ...
    watch(
      () => $route.params.service_version,
      async (serviceVersionId) => {
        if (serviceVersionId) {
          isAllowedToRegister.value = await canUserAccess({
            service: "konnect",
            action: "#consume",
      ...
      if (nonProxiedOperations.length) {
        const linkedOperation = nonProxiedOperations.find((operation) => {
          if (operation.operationId) {
            ...
            const generatedOperationId = idFromPathMethod(
              operation.path,
              operation.method
            );
    ...
    function downloadSpecContents(): void {
      ...
      element.setAttribute(
        ...
        "data:text/plain;charset=utf-8," +
          encodeURIComponent(specContents.value)
      );
    ...
    async function fetchSpec(version: string) {
      ...
      return await portalApiV2.value.service.versionsApi
        .getProductVersionSpec({
    ...
    async function loadSwagger() {
      ...
      if (!currentServiceVersion && serviceVersions.value.size > 0) {
        ...
        const serviceVersion = Array.from(serviceVersions.value.values()).find(
          (serviceVersion) => {
        ...
        $router.replace({
          ...
          params: {
            service_version:
              serviceVersion?.id && encodeURIComponent(serviceVersion?.id),
            service_package: servicePackage,
          },
        });
